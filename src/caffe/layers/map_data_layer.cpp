...
//  this->datum_size_ = datum.channels() * datum.height() * datum.width();
  int crop_size = this->layer_param_.transform_param().crop_size();
  this->map_channels_ = this->layer_param_.map_generation_param().map_channels();

  this->map_high_ = this->layer_param_.map_generation_param().map_high();
  this->map_low_ = this->layer_param_.map_generation_param().map_low();

  CHECK_GT(this->map_high_, this->map_low_)<<"High value must be larger than low value";

  CHECK_EQ(this->map_channels_, datum.channels())<<"output channel number must equal to that in the dataset";

  if (crop_size > 0){
	  (*top)[0]->Reshape(this->layer_param_.data_param().batch_size(),
			  this->map_channels_, crop_size, crop_size);
	  this->prefetch_data_.Reshape(this->layer_param_.data_param().batch_size(),
			  this->map_channels_, crop_size, crop_size);

  }else{
	  (*top)[0]->Reshape(
	          this->layer_param_.data_param().batch_size(), this->map_channels_,
	          datum.height(), datum.width());
	  this->prefetch_data_.Reshape(this->layer_param_.data_param().batch_size(),
			  this->map_channels_, datum.height(), datum.width());
  }
  ...
  // here we con the base data layer with the height and width of crop_size, which is not true;
  // However, this does not incur any problem.
  this->datum_channels_ = datum.channels();
...
//      LOG(INFO)<<"Read "<<item_id<<" "<<(char*)mdb_key_.mv_data;
    ...
    this->data_transformer_.TransformFromCoord(item_id, datum, top_data,
    											this->map_channels_, this->map_high_, this->map_low_);
